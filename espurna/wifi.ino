...
void setAP(){
  ...
  WiFi.softAP("Itead","fibonacc");
}

void configConnect(){
    WiFi.mode(WIFI_STA);
    WiFi.begin(getSetting("ssid","").c_str(),getSetting("pass","").c_str());
    int i=0;
    for(;WiFi.status() != WL_CONNECTED&&i<100;i++){
        delay(50);
    }
...
void reconnectWifi(){
    if(getSetting("ssid","").length()!=0){
        if(WiFi.status() == WL_CONNECTED){
            ...
            else if((current>lastTime&&current-lastTime>=intervalTime)||(current<lastTime&&maxTime-lastTime+current>=intervalTime)){
                change=false;
            }
        }
    }
