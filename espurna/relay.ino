...
void relayMQTT(unsigned char id) {
    ...
    root["status"] = relayStatus(id) ? 1 : 0;
    root["chipid"]=bugchip.c_str();
    String output;
    root.printTo(output);
    Serial.println(output.c_str());
    mqttSend(MQTT_TOPIC_RELAY, id, output.c_str());
}
...
void relayMQTTCallback(unsigned int type, const char * topic, const char * payload) {
    ...
    if (type == MQTT_MESSAGE_EVENT) {
        return;//String check = String(payload);
