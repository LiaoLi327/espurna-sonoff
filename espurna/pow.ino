...
void powLoop() {
    ...
    if (millis() - last_update > POW_UPDATE_INTERVAL) {
        ...
        root.printTo(output);
        output=output+"\n";
        wsSend(output.c_str());/*发送目的    POW_UPDATE_INTERVAL更新周期 */


        /*Serial.println("powActivePower: ");
        Serial.println(power);
        Serial.println("powCurrent: ");
        Serial.println(current);
        Serial.println("powVoltage: ");
        Serial.println(voltage);
        Serial.print("IP address: ");
        Serial.println(WiFi.localIP());
      /*  Serial.println("powApparentPower: "+apparent);
        Serial.println("powActivePower: "+reactive);
        Serial.println("powPowerFactor: "+factor);*/
        ...
        if (--report_count == 0) {
            ...
            double energy_inc = (double) power * POW_REPORT_EVERY * POW_UPDATE_INTERVAL / 1000.0 / 3600.0 * 0.0169;
            char energy_buf[11];
            dtostrf(energy_inc, 11, 7, energy_buf);
            char *e = energy_buf;
          ...
          /*  mqttSend(getSetting("powPowerTopic", POW_POWER_TOPIC).c_str(), String(power).c_str());
            ...
            root2["power"]=power;
            root2["energy"]=e;
            root2["current"]=current;
            root2["voltage"]=voltage;
            //root2["apparent"]=apparent;
          //root2["reactive"]=reactive;
          //  root2["factor"]=factor;
            root2["chipid"]=bugchip.c_str();
            String output2;
            root2.printTo(output2);
            mqttSend(String("power").c_str(),output2.c_str());
