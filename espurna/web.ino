...
bool connect(byte attempts)
{

  for(byte count=1;count<=attempts;count++)
  {
...
void _onConnect(AsyncWebServerRequest *request) {


  if(WiFi.status() == WL_CONNECTED)
  {
    setSetting("ssid",name);
    setSetting("pass",pass);
    powEnable(true);
    request->send(200, "text/plain",String(ESP.getChipId()));
  }
  else
  {
    if(request->hasParam("ssid")&&request->hasParam("pass"))
    {
      AsyncWebParameter* request_name=request->getParam("ssid");
      AsyncWebParameter* request_pass=request->getParam("pass");
      name=new char[request_name->value().length()+1];
      pass=new char[request_pass->value().length()+1];
      strcpy_P(name, request_name->value().c_str());
      strcpy_P(pass,request_pass->value().c_str());
      wificonnect=true;
    }
    else
    {
...
void _onClose(AsyncWebServerRequest *request) {

  setStatus(false);
  request->send(200, "text/plain","close");
}

void _onOpen(AsyncWebServerRequest *request) {

  setStatus(true);
  request->send(200, "text/plain","open");
}

void _onRelayTimer(AsyncWebServerRequest *request) {

      if(request->hasParam("start")&&request->hasParam("end"))
      {
          startTime=atol(request->getParam("start")->value().c_str());
          stopTime=atol(request->getParam("end")->value().c_str());
...
void webSetup() {
    ...
    _server->on("/connect",HTTP_GET,_onConnect);
    _server->on("/close",HTTP_GET,_onClose);
    _server->on("/open",HTTP_GET,_onOpen);
    _server->on("/timer",HTTP_GET,_onRelayTimer);
    // Serve static files
    #if not EMBEDDED_WEB
...
void webLoop()
{
  ...
  if(startChange&&(millis()-restartTime)>=30000)
  {
    startChange=false;
    WiFi.disconnect();
    WiFi.mode(WIFI_STA);
